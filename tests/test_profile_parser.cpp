...
QStringList recordStrings(const ProfileRecord &record) {
  QStringList strings{
      record.opaqueId, record.sourcePath,    record.launchPath, record.name,
      record.server,   record.labelsDisplay, record.protocol,
  };
...
void ProfileParserTest::decodesEscapesAndSplitsLabels() {
  ...
  const auto result =
      parseRemminaProfile(sourcePath, QStringLiteral("/launch/escaped.remmina"), QStringLiteral("escaped-id"));

  QVERIFY(std::holds_alternative<ProfileRecord>(result));
  const ProfileRecord &record = std::get<ProfileRecord>(result);
  QCOMPARE(record.name, QStringLiteral("Escaped Office"));
  QCOMPARE(record.server, QStringLiteral("rdp.example.test:3389?mode=fast session=primary"));
...
void ProfileParserTest::returnsEmptyMissingOptionalFields() {
  ...
  const QString path = writeFile(directory, u"name-only.remmina", QByteArray("[remmina]\nname=  Visible  \n"));

  const auto result =
      parseRemminaProfile(path, QStringLiteral("/launch/name-only.remmina"), QStringLiteral("name-only"));
...
void ProfileParserTest::rejectsMissingOrWhitespaceOnlyName() {
  ...
  const QString missing =
      writeFile(directory, u"missing-name.remmina", QByteArray("[remmina]\nserver=host.example.test\n"));
  const QString whitespace = writeFile(directory, u"empty-name.remmina", QByteArray("[remmina]\nname=\\s\\t\n"));

  const auto missingResult = parseRemminaProfile(missing, QStringLiteral("/launch/missing"), QStringLiteral("missing"));
  const auto whitespaceResult =
      parseRemminaProfile(whitespace, QStringLiteral("/launch/whitespace"), QStringLiteral("whitespace"));
...
void ProfileParserTest::classifiesWrongGroupAndMalformedInput() {
  QTemporaryDir directory;
  QVERIFY(directory.isValid());
  const QString wrongGroup = writeFile(directory, u"wrong-group.remmina", QByteArray("[other]\nname=Other\n"));
  const QString malformed = QFINDTESTDATA("fixtures/malformed.remmina");
  QVERIFY(!malformed.isEmpty());

  const auto wrongGroupResult =
      parseRemminaProfile(wrongGroup, QStringLiteral("/launch/wrong"), QStringLiteral("wrong"));
  const auto malformedResult =
      parseRemminaProfile(malformed, QStringLiteral("/launch/malformed"), QStringLiteral("malformed"));
...
void ProfileParserTest::classifiesUnreadableAndNonregularInput() {
  ...
  const auto missing = parseRemminaProfile(directory.filePath(QStringLiteral("missing.remmina")),
                                           QStringLiteral("/launch/missing"), QStringLiteral("missing"));
  const auto directoryResult =
      parseRemminaProfile(directory.path(), QStringLiteral("/launch/directory"), QStringLiteral("directory"));
...
void ProfileParserTest::rejectsInvalidUtf8AndOversizedInput() {
  ...
  const auto invalidResult =
      parseRemminaProfile(invalidPath, QStringLiteral("/launch/invalid"), QStringLiteral("invalid"));
  const auto oversizedResult =
      parseRemminaProfile(oversizedPath, QStringLiteral("/launch/oversized"), QStringLiteral("oversized"));
...
void ProfileParserTest::doesNotRetainPrivateValuesInRecordOrDiagnostics() {
  ...
  const auto validResult = parseRemminaProfile(valid, QStringLiteral("/launch/valid"), QStringLiteral("valid-id"));
  const auto escapedResult =
      parseRemminaProfile(escaped, QStringLiteral("/launch/escaped"), QStringLiteral("escaped-id"));
  const auto malformedResult =
      parseRemminaProfile(malformed, QStringLiteral("/launch/malformed"), QStringLiteral("malformed-id"));
  ...
  verifyPrivateValuesAbsent(std::get<ProfileRecord>(escapedResult),
                            {QStringLiteral("escaped-password-secret"), QStringLiteral("escaped-gateway-secret"),
                             QStringLiteral("escaped-username-secret"), QStringLiteral("escaped-notes-secret")});
  QVERIFY(std::holds_alternative<ProfileParseError>(malformedResult));
  QVERIFY(std::get<ProfileParseError>(malformedResult) == ProfileParseError::Malformed);
}
