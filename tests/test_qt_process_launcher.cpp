...
void QtProcessLauncherTest::launchesDetachedWithExactArgumentsAndEnvironment()
{
    ...
    const QString recordPath =
        QDir(temporaryDirectory.path()).filePath(QStringLiteral("launch record.bin"));
    const QString shellSideEffect =
        QDir(temporaryDirectory.path()).filePath(QStringLiteral("must-not-exist"));
    ...
    environment.insert(QStringLiteral("LAUNCH_MARKER"),
                       QStringLiteral("marker with spaces;$()\n雪"));
...
void QtProcessLauncherTest::reportsMissingProgramAsFailure()
{
    ...
    QVERIFY(temporaryDirectory.isValid());
    const LaunchRequest request{
        .program = QDir(temporaryDirectory.path())
                       .filePath(QStringLiteral("definitely-not-a-program")),
        .arguments = {},
