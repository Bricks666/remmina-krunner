...
void KeyFileReaderTest::readsOnlyAllowlistedValuesFromExactSection() {
  ...
  const auto result = readAllowedKeyFileValues(fixture, QStringView(u"remmina"), searchableKeys());

  QVERIFY(result.has_value());
  QCOMPARE(result->size(), 4);
  QCOMPARE(result->value(QStringLiteral("name")), QStringLiteral("Zürich Office"));
  QCOMPARE(result->value(QStringLiteral("server")), QStringLiteral("rdp.example.test:3389?token=visible=part"));
  QCOMPARE(result->value(QStringLiteral("labels")), QStringLiteral("Finance East, Operations, Zürich"));
  ...
  verifyPrivateValuesAbsent(*result,
                            {QStringLiteral("synthetic-password-secret"), QStringLiteral("synthetic-gateway-secret"),
                             QStringLiteral("synthetic-username-secret"), QStringLiteral("synthetic-notes-secret")});
}
...
void KeyFileReaderTest::acceptsSyntacticWhitespaceAroundExactSectionHeader() {
  ...
  const QString path =
      writeFile(directory, u"section-whitespace.remmina", QByteArray(" \t[remmina]\t \nname=Visible\n"));

  const auto result = readAllowedKeyFileValues(path, QStringView(u"remmina"), {QStringLiteral("name")});
...
void KeyFileReaderTest::matchesVerifiedGlibWhitespace_data() {
  ...
  QTest::newRow("form-feed-before-group")
      << QByteArray("\f[remmina]\nname=Visible\n") << true << true << QStringLiteral("Visible");
  QTest::newRow("form-feed-before-key") << QByteArray("[remmina]\n\fname=Visible\n") << true << true
  ...
  QTest::newRow("vertical-tab-before-key") << QByteArray("[remmina]\n\vname=Visible\n") << true << false << QString();
  QTest::newRow("vertical-tab-before-equals")
      << QByteArray("[remmina]\nname\v=Visible\n") << true << false << QString();
  QTest::newRow("vertical-tab-after-equals")
      << QByteArray("[remmina]\nname=\vVisible\n") << true << true << QStringLiteral("\vVisible");
  QTest::newRow("form-feed-after-group") << QByteArray("[remmina]\f\nname=Visible\n") << false << false << QString();
  QTest::newRow("vertical-tab-after-group") << QByteArray("[remmina]\v\nname=Visible\n") << false << false << QString();
  QTest::newRow("trailing-form-feed-and-vertical-tab")
      << QByteArray("[remmina]\nname=Visible\v\f\n") << true << true << QStringLiteral("Visible\v\f");
}
...
void KeyFileReaderTest::rejectsSemicolonComments() {
  QTemporaryDir directory;
  QVERIFY(directory.isValid());
  const QString beforeGroup = writeFile(directory, u"semicolon-before.remmina",
                                        QByteArray("; synthetic-semicolon-secret\n[remmina]\nname=Visible\n"));
  const QString insideGroup = writeFile(directory, u"semicolon-inside.remmina",
                                        QByteArray("[remmina]\n; synthetic-semicolon-secret\nname=Visible\n"));

  QVERIFY(!readAllowedKeyFileValues(beforeGroup, QStringView(u"remmina"), {QStringLiteral("name")}).has_value());
  QVERIFY(!readAllowedKeyFileValues(insideGroup, QStringView(u"remmina"), {QStringLiteral("name")}).has_value());
}

void KeyFileReaderTest::handlesCrLfFirstEqualsUnicodeAndEscapes() {
  QTemporaryDir directory;
  QVERIFY(directory.isValid());
  const QString path = writeFile(directory, u"escaped-crlf.remmina",
                                 QByteArray("# comment\r\n"
                                            "   # indented comment\r\n"
                                            "\t \r\n"
                                            "[other]\r\n"
  ...
  const auto result = readAllowedKeyFileValues(path, QStringView(u"remmina"), searchableKeys());

  QVERIFY(result.has_value());
  QCOMPARE(result->value(QStringLiteral("name")), QStringLiteral("Caf Name\nLine\tTab\rCarriage\\Slash "));
...
void KeyFileReaderTest::preservesMeaningfulTrailingValueWhitespace() {
  QTemporaryDir directory;
  QVERIFY(directory.isValid());
  const QString path = writeFile(directory, u"trailing-space.remmina", QByteArray("[remmina]\nname=  Visible \t\n"));
...
void KeyFileReaderTest::usesLastDuplicateAllowedValue() {
  QTemporaryDir directory;
  QVERIFY(directory.isValid());
  const QString path = writeFile(directory, u"duplicate.remmina",
                                 QByteArray("[remmina]\n"
                                            "name=First\n"
                                            "name=Second\n"));
...
void KeyFileReaderTest::letsValidLastDuplicateReplaceInvalidEscape() {
  QTemporaryDir directory;
  QVERIFY(directory.isValid());
  const QString path = writeFile(directory, u"duplicate-invalid.remmina",
                                 QByteArray("[remmina]\n"
                                            "name=Invalid\\q\n"
                                            "name=Visible\n"));
...
void KeyFileReaderTest::ignoresUnknownValuesWithoutDecodingEscapes() {
  QTemporaryDir directory;
  QVERIFY(directory.isValid());
  const QString path = writeFile(directory, u"unknown-escape.remmina",
                                 QByteArray("[remmina]\n"
                                            "name=Visible\n"
...
void KeyFileReaderTest::rejectsUnknownAndDanglingEscapesForAllowedValues() {
  QTemporaryDir directory;
  QVERIFY(directory.isValid());
  const QString unknown = writeFile(directory, u"unknown.remmina", QByteArray("[remmina]\nname=Bad\\qEscape\n"));
  const QString dangling = writeFile(directory, u"dangling.remmina", QByteArray("[remmina]\nname=Dangling\\\n"));

  QVERIFY(!readAllowedKeyFileValues(unknown, QStringView(u"remmina"), {QStringLiteral("name")}).has_value());
  QVERIFY(!readAllowedKeyFileValues(dangling, QStringView(u"remmina"), {QStringLiteral("name")}).has_value());
}
...
void KeyFileReaderTest::rejectsOversizedLines() {
  ...
  QVERIFY(!readAllowedKeyFileValues(path, QStringView(u"remmina"), {QStringLiteral("name")}).has_value());
}
...
void KeyFileReaderTest::rejectsWrongOrMissingSection() {
  QTemporaryDir directory;
  QVERIFY(directory.isValid());
  const QString path = writeFile(directory, u"wrong-section.remmina", QByteArray("[not-remmina]\nname=Other\n"));
...
void KeyFileReaderTest::rejectsUnreadableAndNonregularInputs() {
  ...
  QVERIFY(!readAllowedKeyFileValues(directory.filePath(QStringLiteral("missing.remmina")), QStringView(u"remmina"),
                                    {QStringLiteral("name")})
               .has_value());
  QVERIFY(!readAllowedKeyFileValues(directory.path(), QStringView(u"remmina"), {QStringLiteral("name")}).has_value());
  ...
  const auto fifoResult = key_file_reader_detail::readAllowedKeyFileValuesWithError(fifoPath, QStringView(u"remmina"),
                                                                                    {QStringLiteral("name")});
  ...
  const auto deviceResult = key_file_reader_detail::readAllowedKeyFileValuesWithError(
      QStringLiteral("/dev/null"), QStringView(u"remmina"), {QStringLiteral("name")});
...
void KeyFileReaderTest::followsSymlinkToRegularInput() {
  QTemporaryDir directory;
  QVERIFY(directory.isValid());
  const QString target = writeFile(directory, u"target.remmina", QByteArray("[remmina]\nname=Visible\n"));
  const QString link = directory.filePath(QStringLiteral("link.remmina"));
  QVERIFY(QFile::link(target, link));

  const auto result = readAllowedKeyFileValues(link, QStringView(u"remmina"), {QStringLiteral("name")});
...
void KeyFileReaderTest::rejectsMalformedSectionsAndLines() {
  ...
  const QString section =
      writeFile(directory, u"bad-section.remmina", QByteArray("[remmina] trailing\nname=Visible\n"));
  const QString line = writeFile(directory, u"bad-line.remmina", QByteArray("[remmina]\nnot-a-key-value\n"));
  const QString key = writeFile(directory, u"bad-key.remmina", QByteArray("[remmina]\nbad[key=value\n"));

  QVERIFY(!readAllowedKeyFileValues(section, QStringView(u"remmina"), {QStringLiteral("name")}).has_value());
  QVERIFY(!readAllowedKeyFileValues(line, QStringView(u"remmina"), {QStringLiteral("name")}).has_value());
  QVERIFY(!readAllowedKeyFileValues(key, QStringView(u"remmina"), {QStringLiteral("name")}).has_value());
}
