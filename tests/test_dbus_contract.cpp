...
public:
    CatalogResult records(const RemminaInstance &) override
    {
        return QList<ProfileRecord>{};
    }
    std::optional<ProfileRecord> resolve(QStringView, QStringView) const override
    {
        return std::nullopt;
    }
...
public slots:
    RemminaKRunner::RemoteMatch EchoMatch(RemminaKRunner::RemoteMatch match)
    {
        return match;
    }

    RemminaKRunner::RunnerActions EchoActions(RemminaKRunner::RunnerActions actions)
    {
        return actions;
    }
};
...
QDBusMessage await(QDBusPendingCall call)
{
    ...
    if (!watcher.isFinished()) {
        return QDBusMessage::createError(QDBusError::Timeout,
                                         QStringLiteral("private round-trip timed out"));
    }
...
void DbusContractTest::metatypesHaveExactSignaturesAndRoundTrip()
{
    ...
    QCOMPARE(QByteArray(QDBusMetaType::typeToSignature(
                 QMetaType::fromType<RemminaKRunner::RemoteMatches>())),
             QByteArray("a(sssida{sv})"));
    QCOMPARE(QByteArray(QDBusMetaType::typeToSignature(
                 QMetaType::fromType<RemminaKRunner::RunnerActions>())),
             QByteArray("a(sss)"));

    const RemminaKRunner::RemoteMatch original{
        QStringLiteral("opaque"), QStringLiteral("Visible"), QStringLiteral("icon"), 50, 0.75,
    ...
    const QString serviceName = QStringLiteral("org.example.RemminaKRunner.RoundTrip.p%1")
                                    .arg(QCoreApplication::applicationPid());
    RoundTripRelay relay;
    QVERIFY(bus.registerService(serviceName));
    QVERIFY(bus.registerObject(QStringLiteral("/relay"), &relay,
                               QDBusConnection::ExportAllSlots));
    QDBusInterface interface(serviceName,
                             QStringLiteral("/relay"),
                             QStringLiteral("org.example.RemminaKRunner.RoundTrip"),
                             bus);
    QVERIFY(interface.isValid());
    const QDBusMessage matchReply = await(interface.asyncCall(
        QStringLiteral("EchoMatch"), QVariant::fromValue(original)));
    QCOMPARE(matchReply.type(), QDBusMessage::ReplyMessage);
    QCOMPARE(matchReply.arguments().size(), 1);
    const QDBusArgument encodedMatch =
    ...
    const RemminaKRunner::RunnerActions actions{{QStringLiteral("id"),
                                                  QStringLiteral("Text"),
                                                  QStringLiteral("icon")}};
    const QDBusMessage actionsReply = await(interface.asyncCall(
        QStringLiteral("EchoActions"), QVariant::fromValue(actions)));
    QCOMPARE(actionsReply.type(), QDBusMessage::ReplyMessage);
    QCOMPARE(actionsReply.arguments().size(), 1);
    const QDBusArgument encodedActions =
        qvariant_cast<QDBusArgument>(actionsReply.arguments().constFirst());
...
{
    static_assert(noexcept(std::declval<RemminaKRunner::RunnerService &>().Match(
        std::declval<const QString &>())));
    static_assert(noexcept(std::declval<RemminaKRunner::RunnerService &>().Actions()));
    static_assert(noexcept(std::declval<RemminaKRunner::RunnerService &>().Run(
        std::declval<const QString &>(), std::declval<const QString &>())));
    ...
    static_assert(noexcept(std::declval<RemminaKRunner::RunnerService &>().SetActivationToken(
        std::declval<const QString &>())));
    ...
    QCOMPARE(slotSignatures,
             QStringList({QStringLiteral("Match(QString)"),
                          QStringLiteral("Actions()"),
                          QStringLiteral("Run(QString,QString)"),
                          QStringLiteral("Teardown()"),
                          QStringLiteral("Config()"),
                          QStringLiteral("SetActivationToken(QString)")}));
}
...
void DbusContractTest::liveServiceExportsAndAnswersEveryContractMethod()
{
    ...
    QDBusConnection bus = QDBusConnection::sessionBus();
    QVERIFY(bus.isConnected());
    const QString serviceName = QStringLiteral("org.example.RemminaKRunner.Contract.p%1")
                                    .arg(QCoreApplication::applicationPid());
    QVERIFY(bus.registerService(serviceName));
    QVERIFY(bus.registerObject(QStringLiteral("/runner"), &service,
                               QDBusConnection::ExportAllSlots));

    QDBusInterface introspection(serviceName,
                                 QStringLiteral("/runner"),
                                 QStringLiteral("org.freedesktop.DBus.Introspectable"),
                                 bus);
    const QDBusMessage introspectionReply =
        await(introspection.asyncCall(QStringLiteral("Introspect")));
    ...
    QCOMPARE(methods,
             QStringList({QStringLiteral("Match"),
                          QStringLiteral("Actions"),
                          QStringLiteral("Run"),
                          QStringLiteral("Teardown"),
                          QStringLiteral("Config"),
    ...
    QDBusInterface runner(serviceName,
                          QStringLiteral("/runner"),
                          QStringLiteral("org.kde.krunner1"),
                          bus);
    ...
    QCOMPARE(actionsReply.type(), QDBusMessage::ReplyMessage);
    const QDBusArgument actionsArgument =
        qvariant_cast<QDBusArgument>(actionsReply.arguments().constFirst());
