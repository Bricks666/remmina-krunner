...
void SelectionPolicyTest::fallsBackByKindPriorityAfterRemoval() {
  ...
  const SelectionDecision snapFallback = validateSelection(snapOnly, QStringLiteral("flatpak:removed.Application"));
...
void SelectionPolicyTest::handlesEmptyScan() {
  ...
  const SelectionDecision cleared = validateSelection(noInstances, QStringLiteral("native:/removed/remmina"));
...
void SelectionPolicyTest::ignoresDisplayMetadataWhenSelectingStableId() {
  ...
  QList<RemminaInstance> instances{
      makeInstance(QStringLiteral("native:/usr/bin/remmina"), InstanceKind::Native, QStringLiteral("Remmina 1.4.40")),
