...
void RemminaRunnerConfigTest::loadPopulatesPackagingIdentityAndRescansEveryTime()
{
    ...
    QCOMPARE(combo(module)->currentData().toString(),
...
void RemminaRunnerConfigTest::emptyAndFailedScansDisableTheControlWithClearStatus()
{
    ...
    QCOMPARE(status(failedModule)->text(),
             QStringLiteral("Could not scan for Remmina installations."));
...
void RemminaRunnerConfigTest::savePersistsOnlyOnApplyAndKeepsFailureDirty()
{
    ...
    QCOMPARE(status(failureModule)->text(),
             QStringLiteral("Could not save the Remmina installation selection."));
...
void RemminaRunnerConfigTest::noJsonPluginFactoryLoadsStandardConstructorInIsolation()
{
    ...
    QComboBox *instanceCombo =
        module->widget()->findChild<QComboBox *>(QStringLiteral("instanceCombo"));
    QLabel *statusLabel =
        module->widget()->findChild<QLabel *>(QStringLiteral("statusLabel"));
