...
public:
  explicit FakeScanSource(QList<InstanceScanResult> results = {}) : results_(std::move(results)) {}
...
public:
  explicit FakeSelectionStore(QString selected = {}) : persistedId(std::move(selected)) {}
...
void InstanceRegistryTest::firstScanChoosesPriorityDefaultAndPersists() {
  ...
  const RemminaInstance native = makeInstance(QStringLiteral("native:/usr/bin/remmina"), InstanceKind::Native);
  const RemminaInstance flatpak =
      makeInstance(QStringLiteral("flatpak:user:org.remmina.Remmina/x86_64/stable"), InstanceKind::Flatpak);
...
void InstanceRegistryTest::partialErrorsRetainSuccessfulInstancesAndSelection() {
  const RemminaInstance native = makeInstance(QStringLiteral("native:/usr/bin/remmina"), InstanceKind::Native);
  FakeScanSource source(
      {{.instances = {native}, .failedBackends = {QStringLiteral("flatpak"), QStringLiteral("snap")}}});
  FakeSelectionStore store;
  InstanceRegistry registry(source, store);
  ...
  QCOMPARE(snapshot.selectedId, native.id);
  QCOMPARE(snapshot.failedBackends, QStringList({QStringLiteral("flatpak"), QStringLiteral("snap")}));
}
...
void InstanceRegistryTest::unrelatedBackendFailureDoesNotBlockRepair() {
  const RemminaInstance native = makeInstance(QStringLiteral("native:/usr/bin/remmina"), InstanceKind::Native);
  const QString removedFlatpak = QStringLiteral("flatpak:user:org.remmina.Remmina/x86_64/removed");
  FakeScanSource source({{.instances = {native}, .failedBackends = {QStringLiteral("snap")}}});
...
void InstanceRegistryTest::backendNameInsideUnknownPrefixDoesNotBlockRepair() {
  const RemminaInstance native = makeInstance(QStringLiteral("native:/usr/bin/remmina"), InstanceKind::Native);
  FakeScanSource source({{.instances = {native}, .failedBackends = {QStringLiteral("flatpak")}}});
...
void InstanceRegistryTest::manualSelectionPersistsEphemeralBackendFailureFallback() {
  const RemminaInstance native = makeInstance(QStringLiteral("native:/usr/bin/remmina"), InstanceKind::Native);
  const QString savedFlatpak = QStringLiteral("flatpak:user:org.remmina.Remmina/x86_64/stable");
  FakeScanSource source({{.instances = {native}, .failedBackends = {QStringLiteral("flatpak")}}});
