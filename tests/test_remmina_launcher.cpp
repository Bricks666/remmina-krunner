...
RemminaInstance makeInstance(QString id = QStringLiteral("native:/usr/bin/remmina"),
                             QString executable = QStringLiteral("/usr/bin/remmina"), QStringList prefix = {}) {
  return {
      .id = std::move(id),
...
public:
  [[nodiscard]] std::optional<ProfileRecord> resolve(QStringView expectedInstanceId,
    ...
    return hasRecord && expectedInstanceId == recordInstanceId ? std::optional<ProfileRecord>{record} : std::nullopt;
  }
...
struct Harness {
  Harness() : launcher(registry, catalog, process, notifier, [this] { return environment; }) {
...
void RemminaLauncherTest::buildsExactNativeFlatpakAndSnapConnectCommands() {
  ...
  const QList<Case> cases{
      {QStringLiteral("native:/opt/remmina/bin/remmina"), QStringLiteral("/opt/remmina/bin/remmina"), {}},
      {QStringLiteral("flatpak:user:org.remmina.Remmina/x86_64/stable"),
       QStringLiteral("/usr/bin/flatpak"),
       {QStringLiteral("--user"), QStringLiteral("run"), QStringLiteral("org.remmina.Remmina/x86_64/stable")}},
      {QStringLiteral("flatpak:system:org.remmina.Remmina/x86_64/stable"),
       QStringLiteral("/usr/bin/flatpak"),
       {QStringLiteral("--system"), QStringLiteral("run"), QStringLiteral("org.remmina.Remmina/x86_64/stable")}},
      {QStringLiteral("flatpak:lab:org.remmina.Remmina/x86_64/beta"),
       QStringLiteral("/usr/bin/flatpak"),
       {QStringLiteral("--installation=lab"), QStringLiteral("run"),
  ...
  for (const Case &testCase : cases) {
    Harness harness;
    harness.registry.value.instances = {makeInstance(testCase.id, testCase.program, testCase.prefix)};
    harness.registry.value.selectedId = testCase.id;
    harness.catalog.record =
        makeRecord(QStringLiteral("opaque-safe-id"), QStringLiteral("/launch/profile one.remmina"));
    harness.catalog.recordInstanceId = testCase.id;

    QCOMPARE(harness.launcher.connect(QStringLiteral("opaque-safe-id")), RemminaLaunchResult::Started);
...
void RemminaLauncherTest::createsWithPrefixWithoutConsultingCatalog() {
  Harness harness;
  const QStringList prefix{QStringLiteral("--installation=work station"), QStringLiteral("run"),
                           QStringLiteral("org.remmina.Remmina/x86_64/stable")};
  harness.registry.value.instances = {
      makeInstance(QStringLiteral("flatpak:work station:ref"), QStringLiteral("/usr/bin/flatpak"), prefix)};
...
void RemminaLauncherTest::preservesHostileValuesAsInertArguments() {
  Harness harness;
  const QString hostileProgram = QStringLiteral("/opt/Remmina dir/remmina;$(program)`tick`\\line\n雪");
  ...
  const QString hostilePath = QStringLiteral("/absolute path/quote\"single';$(profile)`tick`\\line\n"
                                             "プロファイル.remmina");
  harness.registry.value.instances = {makeInstance(QStringLiteral("hostile-id"), hostileProgram, hostilePrefix)};
  ...
  QCOMPARE(harness.launcher.connect(QStringLiteral("opaque")), RemminaLaunchResult::Started);
...
void RemminaLauncherTest::scopesActivationTokenToEachEnvironmentSnapshot() {
  ...
  harness.environment.insert(QStringLiteral("XDG_ACTIVATION_TOKEN"), QStringLiteral("stale inherited token"));
  ...
  QCOMPARE(withToken.environment.value(QStringLiteral("UNCHANGED_MARKER")), QStringLiteral("present"));
  ...
  QCOMPARE(harness.environment.value(QStringLiteral("XDG_ACTIVATION_TOKEN")), QStringLiteral("stale inherited token"));
  QCOMPARE(harness.notifier.calls, 0);
}
...
void RemminaLauncherTest::normalizesInheritedEnvironmentBeforeRemovingStaleToken() {
  ...
  const ScopedEnvironmentVariable inheritedMarker(QByteArrayLiteral("INHERITED_ENVIRONMENT_MARKER"),
  ...
  RemminaLauncher launcher(registry, catalog, process, notifier,
                           [] { return QProcessEnvironment{QProcessEnvironment::Initialization::InheritFromParent}; });
  ...
  QCOMPARE(environment.value(QStringLiteral("INHERITED_ENVIRONMENT_MARKER")), QStringLiteral("preserved"));
...
void RemminaLauncherTest::rejectsUnavailableOrAmbiguousSelection() {
  const QList<RegistrySnapshot> invalidSnapshots{
      ...
      {.instances = {makeInstance()}, .selectedId = QStringLiteral("missing-id"), .failedBackends = {}},
      {.instances = {makeInstance(QStringLiteral("duplicate")),
                     makeInstance(QStringLiteral("duplicate"), QStringLiteral("/opt/other/remmina"))},
...
void RemminaLauncherTest::rejectsMissingProfilesAfterValidSelection() {
  ...
  for (const QString &id : ids) {
    ...
    QCOMPARE(harness.catalog.expectedInstanceIds, QStringList{QStringLiteral("native:/usr/bin/remmina")});
...
void RemminaLauncherTest::rejectsProfileFromDifferentSelectedInstance() {
  Harness harness;
  harness.registry.value.instances = {
      makeInstance(QStringLiteral("native:instance-a"), QStringLiteral("/opt/instance-a/remmina"))};
  harness.registry.value.selectedId = QStringLiteral("native:instance-a");
  harness.catalog.record = makeRecord(QStringLiteral("same-opaque-text"), QStringLiteral("/profiles/from-b.remmina"));
  harness.catalog.recordInstanceId = QStringLiteral("native:instance-b");

  QCOMPARE(harness.launcher.connect(QStringLiteral("same-opaque-text")), RemminaLaunchResult::MissingProfile);
  QCOMPARE(harness.catalog.calls, 1);
  QCOMPARE(harness.catalog.expectedInstanceIds, QStringList{QStringLiteral("native:instance-a")});
  QCOMPARE(harness.process.calls, 0);
  QCOMPARE(harness.notifier.calls, 1);

  harness.catalog.recordInstanceId = QStringLiteral("native:instance-a");
  QCOMPARE(harness.launcher.connect(QStringLiteral("same-opaque-text")), RemminaLaunchResult::Started);
  QCOMPARE(harness.catalog.calls, 2);
  QCOMPARE(harness.catalog.expectedInstanceIds,
           QStringList({QStringLiteral("native:instance-a"), QStringLiteral("native:instance-a")}));
  QCOMPARE(harness.process.calls, 1);
  QCOMPARE(harness.notifier.calls, 1);
}
...
void RemminaLauncherTest::rejectsInvalidCommandsAndToken() {
  ...
  const QList<Case> cases{
      ...
      {QStringLiteral("/usr/bin/remmina"), {withNul(QStringLiteral("prefix"))}, QStringLiteral("/profile.remmina"), {}},
      {QStringLiteral("/usr/bin/remmina"), {}, {}, {}},
      {QStringLiteral("/usr/bin/remmina"), {}, QStringLiteral("relative/profile.remmina"), {}},
      {QStringLiteral("/usr/bin/remmina"), {}, withNul(QStringLiteral("/profile.remmina")), {}},
      {QStringLiteral("/usr/bin/remmina"), {}, QStringLiteral("/profile.remmina"), withNul(QStringLiteral("token"))},
  };

  for (const Case &testCase : cases) {
    Harness harness;
    harness.registry.value.instances = {makeInstance(QStringLiteral("selected"), testCase.program, testCase.prefix)};
    ...
    QCOMPARE(harness.launcher.connect(QStringLiteral("opaque"), testCase.token), RemminaLaunchResult::StartFailed);
    QCOMPARE(harness.process.calls, 0);
    QCOMPARE(harness.notifier.calls, 1);
  }
}
...
void RemminaLauncherTest::usesOnlyGenericNotificationConstants() {
  ...
  Harness harness;
  harness.process.succeeds = false;
  QCOMPARE(harness.launcher.connect(QStringLiteral("opaque-safe-id"), QStringLiteral("activation-secret")),
...
void RemminaLauncherTest::buildsExactGenericFreedesktopNotificationMessage() {
  const QDBusMessage message = freedesktop_notifier_detail::launchFailureMessage();
  ...
  const QString rendered = message.service() + message.path() + message.interface() + message.member() +
                           arguments.at(0).toString() + arguments.at(2).toString() + arguments.at(3).toString() +
                           arguments.at(4).toString();
  for (const QString &secret : {QStringLiteral("synthetic-secret"), QStringLiteral("/profiles/from-b.remmina"),
                                QStringLiteral("activation-secret")}) {
...
void RemminaLauncherTest::containsDependencyAndNotifierExceptions() {
  ...
  }
  {
    Harness harness;
    harness.catalog.throws = true;
    QCOMPARE(harness.launcher.connect(QStringLiteral("opaque-safe-id")), RemminaLaunchResult::StartFailed);
    QCOMPARE(harness.notifier.calls, 1);
    QCOMPARE(harness.process.calls, 0);
  }
  {
...
void RemminaLauncherTest::consultsCatalogOnlyForConnectAfterValidCurrentInstance() {
  ...
  QCOMPARE(harness.launcher.connect(QStringLiteral("opaque-safe-id")), RemminaLaunchResult::NoInstance);
  QCOMPARE(harness.catalog.calls, 0);
