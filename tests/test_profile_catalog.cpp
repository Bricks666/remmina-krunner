...
ProfileSnapshot makeSnapshot(QList<ProfileRecord> profiles, QList<FileFingerprint> fingerprint,
                             LocatedProfileDirectory directory, DirectoryFingerprint directoryFingerprint = {}) {
  if (directoryFingerprint.canonicalPath.isEmpty() && !directory.hostPath.isEmpty()) {
...
ProfileSnapshot makeSnapshot(QList<ProfileRecord> profiles, LocatedProfileDirectory directory) {
...
public:
  bool replacePaths(const QStringList &requestedPaths, ChangedCallback changed) override {
    ...
    const bool success = nextResult_ >= replaceResults.size() || replaceResults.at(nextResult_++);
    if (callback) {
...
ProfileSnapshot resolvableSnapshot(QStringView instanceId, const QString &directory, const QString &sourcePath,
  ...
  const std::optional<DirectoryFingerprint> identity = profile_repository_detail::inspectDirectory(directory);
  if (!identity.has_value()) {
    qFatal("Catalog test directory lacks an identity");
  }
  const QString opaqueId = profile_repository_detail::opaqueProfileId(instanceId, QStringView(canonicalSource));
  const QFileInfo metadata(canonicalSource);
  return makeSnapshot({makeRecord(opaqueId, sourcePath, std::move(name))},
                      {{.path = canonicalSource,
                        .size = metadata.size(),
                        .modifiedMilliseconds = metadata.lastModified().toMSecsSinceEpoch()}},
                      located(directory), *identity);
}
...
void ProfileCatalogTest::firstLookupLoadsVerifiesAndWatchesCompleteDeterministicSet() {
  FakeRepository repository;
  repository.results = {makeSnapshot({makeRecord(QStringLiteral("valid"))},
                                     {
  ...
  QCOMPARE(watcher.paths, QStringList({
...
void ProfileCatalogTest::sameSessionReusesRecordsAcrossLookups() {
  FakeRepository repository;
  repository.results = {makeSnapshot({makeRecord(QStringLiteral("one"))}, located(QStringLiteral("/profiles/one")))};
  FakeWatcher watcher;
  ProfileCatalog catalog(repository, watcher);
  const RemminaInstance instance = makeInstance(QStringLiteral("native:one"));
...
void ProfileCatalogTest::explicitDirtyDefersReloadUntilNextLookup() {
  FakeRepository repository;
  repository.results = {
      makeSnapshot({makeRecord(QStringLiteral("one"))}, located(QStringLiteral("/profiles/one"))),
      makeSnapshot({makeRecord(QStringLiteral("one"))}, located(QStringLiteral("/profiles/one"))),
      makeSnapshot({makeRecord(QStringLiteral("two"))}, located(QStringLiteral("/profiles/one"))),
      makeSnapshot({makeRecord(QStringLiteral("two"))}, located(QStringLiteral("/profiles/one"))),
  };
  FakeWatcher watcher;
  ProfileCatalog catalog(repository, watcher);
...
void ProfileCatalogTest::watcherCallbackDefersReloadAndReplacesCurrentIds() {
  FakeRepository repository;
  repository.results = {
      makeSnapshot({makeRecord(QStringLiteral("stale"))}, located(QStringLiteral("/profiles/one"))),
      makeSnapshot({makeRecord(QStringLiteral("stale"))}, located(QStringLiteral("/profiles/one"))),
      makeSnapshot({makeRecord(QStringLiteral("fresh"))}, located(QStringLiteral("/profiles/one"))),
      makeSnapshot({makeRecord(QStringLiteral("fresh"))}, located(QStringLiteral("/profiles/one"))),
  };
  FakeWatcher watcher;
  ProfileCatalog catalog(repository, watcher);
...
void ProfileCatalogTest::endSessionForcesRepositoryVerificationEvenWhenClean() {
  FakeRepository repository;
  repository.results = {
      makeSnapshot({makeRecord(QStringLiteral("unchanged"))}, located(QStringLiteral("/profiles/one"))),
      makeSnapshot({makeRecord(QStringLiteral("unchanged"))}, located(QStringLiteral("/profiles/one"))),
  };
  FakeWatcher watcher;
  ProfileCatalog catalog(repository, watcher);
...
void ProfileCatalogTest::instanceSwitchClearsOldStateBeforeLoadingNewScope() {
  FakeRepository repository;
  repository.results = {
      makeSnapshot({makeRecord(QStringLiteral("old"))}, located(QStringLiteral("/profiles/one"))),
      makeSnapshot({makeRecord(QStringLiteral("old"))}, located(QStringLiteral("/profiles/one"))),
      makeSnapshot({makeRecord(QStringLiteral("new"))}, located(QStringLiteral("/profiles/two"))),
      makeSnapshot({makeRecord(QStringLiteral("new"))}, located(QStringLiteral("/profiles/two"))),
  };
  FakeWatcher watcher;
  ProfileCatalog catalog(repository, watcher);
  ...
  repository.beforeLoad = [&](const RemminaInstance &instance) {
    if (instance.id == second.id && secondLoadCount++ == 0) {
      oldStateWasClearedBeforeSecondLoad = watcher.paths.isEmpty() && !catalog.resolve(QStringView(second.id), u"old");
    }
  };
...
void ProfileCatalogTest::resetClearsStateAndRemainsLazy() {
  FakeRepository repository;
  repository.results = {
      makeSnapshot({makeRecord(QStringLiteral("before"))}, located(QStringLiteral("/profiles/one"))),
      makeSnapshot({makeRecord(QStringLiteral("before"))}, located(QStringLiteral("/profiles/one"))),
      makeSnapshot({makeRecord(QStringLiteral("after"))}, located(QStringLiteral("/profiles/one"))),
      makeSnapshot({makeRecord(QStringLiteral("after"))}, located(QStringLiteral("/profiles/one"))),
  };
  FakeWatcher watcher;
  ProfileCatalog catalog(repository, watcher);
...
void ProfileCatalogTest::repositoryErrorsClearStateMapExactlyAndRetry() {
  const QList<std::pair<ProfileRepositoryError, ProfileCatalogError>> cases{
      {ProfileRepositoryError::NoProfileDirectory, ProfileCatalogError::NoProfileDirectory},
      {ProfileRepositoryError::UnreadableDirectory, ProfileCatalogError::UnreadableDirectory},
  };
  ...
  for (const auto &[repositoryError, catalogError] : cases) {
    ...
    repository.results = {
        ...
        makeSnapshot({makeRecord(QStringLiteral("stale"))}, located(QStringLiteral("/profiles/one"))),
        repositoryError,
        makeSnapshot({makeRecord(QStringLiteral("recovered"))}, located(QStringLiteral("/profiles/one"))),
        makeSnapshot({makeRecord(QStringLiteral("recovered"))}, located(QStringLiteral("/profiles/one"))),
    };
    FakeWatcher watcher;
    ProfileCatalog catalog(repository, watcher);
...
void ProfileCatalogTest::repositoryErrorDuringVerificationClearsStateAndRetries() {
  const ProfileSnapshot fresh =
      makeSnapshot({makeRecord(QStringLiteral("fresh"))}, located(QStringLiteral("/profiles/one")));
  const ProfileSnapshot recovered =
      makeSnapshot({makeRecord(QStringLiteral("recovered"))}, located(QStringLiteral("/profiles/one")));
  FakeRepository repository;
  repository.results = {
      fresh,
...
void ProfileCatalogTest::emptySnapshotStillWatchesSelectedDirectory() {
  ...
  const QList<ProfileRecord> records = requireRecords(catalog.records(makeInstance(QStringLiteral("native:empty"))));
...
void ProfileCatalogTest::watcherSetupFailureServesFreshRecordsAndRetriesUntilSafe() {
  FakeRepository repository;
  repository.results = {
      makeSnapshot({makeRecord(QStringLiteral("first"))}, located(QStringLiteral("/profiles/one"))),
      makeSnapshot({makeRecord(QStringLiteral("second"))}, located(QStringLiteral("/profiles/one"))),
  };
  FakeWatcher watcher;
...
void ProfileCatalogTest::snapshotProvenanceWinsWhenInstanceNowPointsElsewhere() {
  ...
  const QString loadedProfile = QDir(loadedDirectory).filePath(QStringLiteral("profile.remmina"));
  const QString currentDirectory = QDir(temporary.path()).filePath(QStringLiteral("b"));
  QVERIFY(QDir().mkpath(currentDirectory));
  FakeRepository repository;
  repository.results = {
      makeSnapshot({makeRecord(QStringLiteral("from-a"))}, {makeFingerprint(loadedProfile)}, located(loadedDirectory))};
  FakeWatcher watcher;
  ProfileCatalog catalog(repository, watcher);
...
void ProfileCatalogTest::handoffMutationInstallsAndVerifiesNewSnapshot() {
  const ProfileSnapshot a =
      makeSnapshot({makeRecord(QStringLiteral("a"))}, {makeFingerprint(QStringLiteral("/profiles/a/profile.remmina"))},
                   located(QStringLiteral("/profiles/a")));
  const ProfileSnapshot b =
      makeSnapshot({makeRecord(QStringLiteral("b"))}, {makeFingerprint(QStringLiteral("/profiles/b/profile.remmina"))},
  ...
  const QList<ProfileRecord> records = requireRecords(catalog.records(makeInstance(QStringLiteral("native:one"))));
...
void ProfileCatalogTest::repeatedHandoffMutationStaysDirtyAndRetries() {
  const ProfileSnapshot a = makeSnapshot({makeRecord(QStringLiteral("a"))}, located(QStringLiteral("/profiles/a")));
  const ProfileSnapshot b = makeSnapshot({makeRecord(QStringLiteral("b"))}, located(QStringLiteral("/profiles/b")));
  const ProfileSnapshot c = makeSnapshot({makeRecord(QStringLiteral("c"))}, located(QStringLiteral("/profiles/c")));
  FakeRepository repository;
...
void ProfileCatalogTest::synchronousWatcherCallbacksCannotBeOverwrittenClean() {
  const ProfileSnapshot snapshot =
      makeSnapshot({makeRecord(QStringLiteral("record"))}, located(QStringLiteral("/profiles/one")));
  FakeRepository repository;
...
void ProfileCatalogTest::resolveRejectsRemovedAndRetargetedSourceWithoutLoading() {
  ...
  const RemminaInstance instance = makeInstance(QStringLiteral("native:one"));
  FakeRepository repository;
  const ProfileSnapshot removableSnapshot = resolvableSnapshot(instance.id, directory, removable);
  const ProfileSnapshot aliasSnapshot = resolvableSnapshot(instance.id, directory, alias);
  repository.results = {
      removableSnapshot,
      removableSnapshot,
      aliasSnapshot,
...
void ProfileCatalogTest::resolveRejectsRetargetedDirectoryWithoutLoading() {
  ...
  const RemminaInstance instance = makeInstance(QStringLiteral("native:one"));
  ProfileSnapshot snapshot = resolvableSnapshot(instance.id, lexicalDirectory, firstSource, QStringLiteral("one"));
  FakeRepository repository;
  repository.results = {snapshot};
  FakeWatcher watcher;
  ProfileCatalog catalog(repository, watcher);
...
void ProfileCatalogTest::repositoryExceptionAfterCleanSnapshotInvalidatesIt() {
  ...
  QVERIFY(temporary.isValid());
  const QString directory = QDir(temporary.path()).filePath(QStringLiteral("profiles"));
  const QString source = writeBytes(QDir(directory).filePath(QStringLiteral("profile.remmina")));
  const RemminaInstance instance = makeInstance(QStringLiteral("native:one"));
...
void ProfileCatalogTest::resetAndDestructorContainWatcherClearExceptions() {
  FakeRepository repository;
  repository.results = {makeSnapshot({makeRecord(QStringLiteral("record"))}, located(QStringLiteral("/profiles/one")))};
  FakeWatcher watcher;
  ProfileCatalog catalog(repository, watcher);
  const RemminaInstance instance = makeInstance(QStringLiteral("native:one"));
  requireRecords(catalog.records(instance));
...
void ProfileCatalogTest::nestedSymlinkRetargetRefreshesActiveSession() {
  ...
  QTemporaryDir temporary;
  QVERIFY(temporary.isValid());
  const QString lexicalParent = QDir(temporary.path()).filePath(QStringLiteral("lexical"));
  const QString nestedParent = QDir(temporary.path()).filePath(QStringLiteral("x"));
  const QString firstTarget = QDir(temporary.path()).filePath(QStringLiteral("target-a"));
  const QString secondTarget = QDir(temporary.path()).filePath(QStringLiteral("target-b"));
  QVERIFY(QDir().mkpath(lexicalParent));
  QVERIFY(QDir().mkpath(nestedParent));
  const QString firstDirectory = QDir(firstTarget).filePath(QStringLiteral("data/remmina"));
  const QString secondDirectory = QDir(secondTarget).filePath(QStringLiteral("data/remmina"));
  ...
  const QString firstLinkTarget = relativeTargets ? QStringLiteral("../target-a") : firstTarget;
  ...
  const QString currentTarget =
      relativeTargets ? QStringLiteral("../x/next/data") : QDir(nestedLink).filePath(QStringLiteral("data"));
  QVERIFY(QFile::link(currentTarget, current));

  RemminaInstance instance = makeInstance(QStringLiteral("native:one"));
  instance.profiles.configHome = QDir(temporary.path()).filePath(QStringLiteral("config"));
  instance.profiles.dataHome = current;
  instance.profiles.legacyHome = QDir(temporary.path()).filePath(QStringLiteral("legacy"));
  ...
  const QString secondLinkTarget = relativeTargets ? QStringLiteral("../target-b") : secondTarget;
