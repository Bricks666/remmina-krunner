...
InvocationResult invoke(const QStringList &arguments, FakeBackend &backend) {
  ...
  if (!outputBuffer.open(QIODevice::WriteOnly) || !errorBuffer.open(QIODevice::WriteOnly)) {
    qFatal("Unable to open test buffers");
  }
  QTextStream output(&outputBuffer);
  QTextStream error(&errorBuffer);
  const int exitCode = RemminaKRunner::runApplication(*QCoreApplication::instance(), arguments, backend, output, error);
  output.flush();
  error.flush();
  return {
      exitCode,
...
void RescanCliTest::rescanRepairsSelectionAndPrintsOnlySafeSummary() {
  ...
  const InvocationResult result = invoke({QStringLiteral("remmina-krunner"), QStringLiteral("--rescan")}, backend);

  QCOMPARE(result.exitCode, 0);
  QCOMPARE(backend.rescanCalls, 1);
  QCOMPARE(backend.serviceCalls, 0);
  QCOMPARE(result.standardError, QString{});
  QCOMPARE(result.standardOutput, QStringLiteral("scan_status=partial\n"
                                                 "instances=3\n"
                                                 "native=1\n"
                                                 "flatpak=1\n"
                                                 "snap=1\n"
...
void RescanCliTest::emptyScanIsSuccessful() {
  ...
  QCOMPARE(backend.serviceCalls, 0);
  QCOMPARE(result.standardError, QString{});
  QCOMPARE(result.standardOutput, QStringLiteral("scan_status=empty\n"
                                                 "instances=0\n"
                                                 "native=0\n"
                                                 "flatpak=0\n"
                                                 "snap=0\n"
...
void RescanCliTest::invalidArgumentsAreRejectedWithoutTouchingBackends() {
  for (const QStringList arguments : {
           ...
           QStringList{QStringLiteral("remmina-krunner"), QStringLiteral("--rescan"), QStringLiteral("extra")},
       }) {
    FakeBackend backend;
    ...
    QCOMPARE(backend.serviceCalls, 0);
    QCOMPARE(result.standardOutput, QString{});
    QCOMPARE(result.standardError, QStringLiteral("Usage: remmina-krunner [--rescan]\n"));
  }
}
...
void RescanCliTest::installedKf6KeepsTheEffectiveTriggerCaseInsensitive() {
  ...
  QTRY_VERIFY_WITH_TIMEOUT(runnerTest.runner->matchRegex().match(QStringLiteral("REM")).hasMatch(), 3000);
  const QRegularExpression effective = runnerTest.runner->matchRegex();
  QVERIFY(effective.isValid());
  for (const QString &accepted :
       {QStringLiteral("rem"), QStringLiteral("REM"), QStringLiteral("rem host"), QStringLiteral("ReM new")}) {
    QVERIFY2(effective.match(accepted).hasMatch(), qPrintable(effective.pattern()));
  }
  for (const QString &rejected : {QStringLiteral("remote"), QStringLiteral("x rem"), QStringLiteral(" rem")}) {
