...
void InstanceScannerTest::discoversNativeExecutablesInPathOrderWithSpaces() {
  ...
  InstanceScanner scanner(probe, nativeEnvironment({temporaryDirectory.path(), secondDirectory}));
  ...
  QCOMPARE(nativeInstances.at(0).id, QStringLiteral("native:") + QFileInfo(firstExecutable).canonicalFilePath());
...
void InstanceScannerTest::rejectsInvalidNativeCandidates() {
  ...
  nonExecutable.close();
  QVERIFY(QFile::setPermissions(nonExecutable.fileName(), QFileDevice::ReadOwner | QFileDevice::WriteOwner));
  ...
  RecordingProcessProbe probe;
  InstanceScanner scanner(probe, nativeEnvironment({nonexistentDirectory, QFileInfo(directoryCandidate).absolutePath(),
                                                    nonExecutableDirectory, brokenLinkDirectory, validDirectory}));

  const QList<RemminaInstance> nativeInstances = instancesOfKind(scanner.scan(), InstanceKind::Native);

  QCOMPARE(nativeInstances.size(), 1);
  QCOMPARE(nativeInstances.constFirst().executable, QFileInfo(validExecutable).canonicalFilePath());
}
...
void InstanceScannerTest::deduplicatesNativeCanonicalPaths() {
  ...
  QCOMPARE(nativeInstances.constFirst().executable,
           QDir::cleanPath(QFileInfo(QDir(aliasDirectory).filePath(QStringLiteral("remmina"))).absoluteFilePath()));
}
...
void InstanceScannerTest::keepsNativeLauncherIdentityAcrossSymlinkRetarget() {
  ...
  const QList<RemminaInstance> firstInstances =
      instancesOfKind(InstanceScanner(firstProbe, nativeEnvironment({launcherDirectory})).scan(), InstanceKind::Native);
  ...
  const QList<RemminaInstance> refreshedInstances = instancesOfKind(
      InstanceScanner(refreshedProbe, nativeEnvironment({launcherDirectory})).scan(), InstanceKind::Native);

  QCOMPARE(refreshedInstances.size(), 1);
  QCOMPARE(refreshedInstances.constFirst().id, firstInstances.constFirst().id);
  QCOMPARE(refreshedInstances.constFirst().executable, firstInstances.constFirst().executable);
}
...
void InstanceScannerTest::excludesSnapLauncherAliasesWithoutSubstringGuessing() {
  ...
  QVERIFY(temporaryDirectory.isValid());
  const QDir root(temporaryDirectory.path());
  const QString snapTarget =
      createExecutable(root.filePath(QStringLiteral("snap-target")), QStringLiteral("snap-command"));
  ...
  QCOMPARE(nativeInstances.size(), 1);
  QCOMPARE(nativeInstances.constFirst().executable, QFileInfo(unrelatedExecutable).canonicalFilePath());
}
...
void InstanceScannerTest::excludesDistinctNativeAliasToSnapCanonicalTarget() {
  ...
  const QString sharedTarget =
      createExecutable(root.filePath(QStringLiteral("shared")), QStringLiteral("snap-command"));
  ...
  const QString snapLauncher = QDir(snapLauncherDirectory).filePath(QStringLiteral("remmina"));
  const QString nativeAlias = QDir(nativeAliasDirectory).filePath(QStringLiteral("remmina"));
  QVERIFY(snapLauncher != nativeAlias);
  QVERIFY(QFile::link(sharedTarget, snapLauncher));
  QVERIFY(QFile::link(sharedTarget, nativeAlias));
  QCOMPARE(QFileInfo(snapLauncher).canonicalFilePath(), QFileInfo(nativeAlias).canonicalFilePath());
  const QString unrelatedExecutable = createExecutable(unrelatedDirectory);
  QVERIFY(!unrelatedExecutable.isEmpty());

  RecordingProcessProbe probe;
  ScanEnvironment environment = nativeEnvironment({nativeAliasDirectory, unrelatedDirectory});
...
void InstanceScannerTest::excludesOnlyExecutablesUnderSnapMountRoot() {
  ...
  const QString underSnapDirectory = QDir(snapMountRoot).filePath(QStringLiteral("remmina/revision/bin"));
  ...
  RecordingProcessProbe probe;
  ScanEnvironment environment = nativeEnvironment({underSnapDirectory, prefixSiblingDirectory, containsSnapDirectory});
  ...
  QCOMPARE(nativeInstances.size(), 2);
  QCOMPARE(nativeInstances.at(0).executable, QFileInfo(prefixSiblingExecutable).canonicalFilePath());
  QCOMPARE(nativeInstances.at(1).executable, QFileInfo(containsSnapExecutable).canonicalFilePath());
}

void InstanceScannerTest::discoversFlatpakInstallationsInDeterministicOrder() {
  QTemporaryDir temporaryDirectory;
  QVERIFY(temporaryDirectory.isValid());
  const QString flatpakTarget = createExecutable(temporaryDirectory.path(), QStringLiteral("flatpak-real"));
  const QString flatpakAlias = QDir(temporaryDirectory.path()).filePath(QStringLiteral("flatpak launcher"));
  QVERIFY(!flatpakTarget.isEmpty());
  QVERIFY(QFile::link(flatpakTarget, flatpakAlias));

  const QByteArray output = "org.remmina.Remmina\torg.remmina.Remmina/x86_64/stable\tzeta\n"
  ...
  RecordingProcessProbe probe;
  probe.expect(QFileInfo(flatpakTarget).canonicalFilePath(), flatpakListArguments(),
  ...
  const InstanceScanResult result = scanner.scan();
  const QList<RemminaInstance> flatpakInstances = instancesOfKind(result, InstanceKind::Flatpak);
  ...
  QCOMPARE(flatpakInstances.at(0).launcherPrefix, QStringList({QStringLiteral("--user"), QStringLiteral("run"),
                                                               QStringLiteral("org.remmina.Remmina/x86_64/beta")}));
  QCOMPARE(flatpakInstances.at(2).launcherPrefix, QStringList({QStringLiteral("--system"), QStringLiteral("run"),
                                                               QStringLiteral("org.remmina.Remmina/x86_64/stable")}));
  QCOMPARE(flatpakInstances.at(3).launcherPrefix,
           QStringList({QStringLiteral("--installation=alpha"), QStringLiteral("run"),
                        QStringLiteral("org.remmina.Remmina/x86_64/stable")}));
...
void InstanceScannerTest::filtersMalformedAndDuplicateFlatpakRows() {
  QTemporaryDir temporaryDirectory;
  QVERIFY(temporaryDirectory.isValid());
  const QString flatpakExecutable = createExecutable(temporaryDirectory.path(), QStringLiteral("flatpak"));
  ...
  const QByteArray output = "\n"
                            ...
                            "org.remmina.Remmina\torg.remmina.Remmina/x86_64/stable\tuser\n";
  RecordingProcessProbe probe;
  probe.expect(QFileInfo(flatpakExecutable).canonicalFilePath(), flatpakListArguments(),
               {.status = ProbeResult::Status::Success, .standardOutput = output});
  ScanEnvironment environment = nativeEnvironment({});
  environment.flatpakExecutable = flatpakExecutable;
  InstanceScanner scanner(probe, std::move(environment));

  const QList<RemminaInstance> flatpakInstances = instancesOfKind(scanner.scan(), InstanceKind::Flatpak);

  QCOMPARE(flatpakInstances.size(), 1);
  QCOMPARE(flatpakInstances.constFirst().id, QStringLiteral("flatpak:user:org.remmina.Remmina/x86_64/stable"));
  QVERIFY(probe.expectationsMet());
}
...
void InstanceScannerTest::rejectsUnsafeFlatpakFields() {
  ...
  QByteArray output("org.remmina.Remmina\torg.remmina.Remmina/x86_64/stable\tuser\n");
  ...
  const QList<RemminaInstance> flatpakInstances =
      instancesOfKind(InstanceScanner(probe, std::move(environment)).scan(), InstanceKind::Flatpak);
...
void InstanceScannerTest::treatsMissingFlatpakAsUnavailable() {
  QTemporaryDir temporaryDirectory;
  QVERIFY(temporaryDirectory.isValid());
  const QString nonExecutable = QDir(temporaryDirectory.path()).filePath(QStringLiteral("flatpak"));
  QFile file(nonExecutable);
  QVERIFY(file.open(QIODevice::WriteOnly));
  file.close();
  QVERIFY(QFile::setPermissions(nonExecutable, QFileDevice::ReadOwner | QFileDevice::WriteOwner));
...
void InstanceScannerTest::isolatesEveryFlatpakProbeFailure() {
  ...
  const QString flatpakExecutable = createExecutable(root.filePath(QStringLiteral("tools")), QStringLiteral("flatpak"));
...
void InstanceScannerTest::discoversSnapThroughRefreshStableLauncher() {
  ...
  const InstanceScanResult firstResult = InstanceScanner(firstProbe, std::move(firstEnvironment)).scan();
  ...
  QCOMPARE(firstSnap.constFirst().executable, QDir::cleanPath(QFileInfo(stableLauncher).absoluteFilePath()));
  ...
  const InstanceScanResult refreshedResult = InstanceScanner(refreshedProbe, std::move(refreshedEnvironment)).scan();

  QCOMPARE(instancesOfKind(refreshedResult, InstanceKind::Snap).constFirst().id, firstSnap.constFirst().id);
...
void InstanceScannerTest::treatsMissingOrNonExecutableSnapAsUnavailable() {
  QTemporaryDir temporaryDirectory;
  QVERIFY(temporaryDirectory.isValid());
  const QString nonExecutable = QDir(temporaryDirectory.path()).filePath(QStringLiteral("remmina"));
  QFile file(nonExecutable);
  QVERIFY(file.open(QIODevice::WriteOnly));
  file.close();
  QVERIFY(QFile::setPermissions(nonExecutable, QFileDevice::ReadOwner | QFileDevice::WriteOwner));
...
void InstanceScannerTest::isolatesSnapResolutionFailureAndOrdersBackendFailures() {
  ...
  QCOMPARE(result.failedBackends, QStringList({QStringLiteral("flatpak"), QStringLiteral("snap")}));
...
void InstanceScannerTest::buildsPackagingSpecificProfileEnvironmentsAndOverallOrder() {
  ...
  probe.expect(QFileInfo(flatpakExecutable).canonicalFilePath(), flatpakListArguments(),
               {.status = ProbeResult::Status::Success,
                .standardOutput = QByteArray("org.remmina.Remmina\torg.remmina.Remmina/x86_64/stable\tnamed\n"
                                             "org.remmina.Remmina\torg.remmina.Remmina/x86_64/stable\tuser\n")});
  ...
  const QString flatpakBase = cleanedChildPath(userHome, QStringLiteral(".var/app/org.remmina.Remmina"));
