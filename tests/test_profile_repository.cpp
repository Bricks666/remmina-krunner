...
QString writeProfile(const QString &directory, QStringView fileName, QStringView profileName = u"Visible") {
  const QString path = QDir(directory).filePath(fileName.toString());
  writeFile(path, QByteArray("[remmina]\nname=") + profileName.toString().toUtf8() +
...
ProfileRecord successfulRecord(const QString &sourcePath, const QString &launchPath, const QString &opaqueId) {
  return {
      .opaqueId = opaqueId,
...
void setModifiedMilliseconds(const QString &path, qint64 milliseconds) {
  ...
  if (!file.open(QIODevice::ReadWrite) ||
      !file.setFileTime(QDateTime::fromMSecsSinceEpoch(milliseconds), QFileDevice::FileModificationTime)) {
...
void ProfileRepositoryTest::filtersSuffixAndFileTypesSortsAndDeduplicatesAliases() {
  ...
  QVERIFY(QFile::link(directory + QStringLiteral("/missing-target"), directory + QStringLiteral("/broken.remmina")));
  ...
  ProfileRepository repository([&calls](const QString &source, const QString &launch, QString opaqueId) {
  ...
  QVERIFY(profile_repository_detail::directoryMatches(snapshot.directory.hostPath, snapshot.directoryFingerprint));
...
void ProfileRepositoryTest::passesFlatpakHostAndLaunchPathsAndOpaqueIdsToParser() {
  ...
  QList<ParseCall> calls;
  ProfileRepository repository([&calls](const QString &sourcePath, const QString &launchPath, QString opaqueId) {
...
void ProfileRepositoryTest::skipsEveryParserErrorAndFingerprintsFailedProfiles() {
  ...
  int parseCount = 0;
  ProfileRepository repository([&parseCount](const QString &source, const QString &launch, QString opaqueId) {
    ++parseCount;
...
void ProfileRepositoryTest::defaultParserLoadsValidAndSkipsMalformedProfiles() {
  ...
  writeFile(directory + QStringLiteral("/malformed.remmina"), QByteArray("[remmina\nname=Private ignored\n"));
...
void ProfileRepositoryTest::capturesSnapDirectoryIdentityAndSymlinkParent() {
  QTemporaryDir temporary;
  QVERIFY(temporary.isValid());
  const QString snapRoot = makeDirectory(temporary.path() + QStringLiteral("/home/tester/snap/remmina"));
  ...
  QCOMPARE(snapshot.directory.hostPath, directory);
  QCOMPARE(snapshot.directoryFingerprint.canonicalPath, QFileInfo(directory).canonicalFilePath());
...
void ProfileRepositoryTest::capturesNestedSymlinkParents() {
  ...
  QTemporaryDir temporary;
  QVERIFY(temporary.isValid());
  const QString lexicalParent = makeDirectory(QDir(temporary.path()).filePath(QStringLiteral("lexical")));
  const QString nestedParent = makeDirectory(QDir(temporary.path()).filePath(QStringLiteral("x")));
  const QString target = makeDirectory(QDir(temporary.path()).filePath(QStringLiteral("target-a")));
  const QString directory = makeDirectory(QDir(target).filePath(QStringLiteral("data/remmina")));
  writeProfile(directory, u"nested.remmina");
  const QString nestedLink = QDir(nestedParent).filePath(QStringLiteral("next"));
  const QString nestedTarget = relativeTargets ? QStringLiteral("./../target-a") : target;
  ...
  instance.profiles.legacyHome = QDir(temporary.path()).filePath(QStringLiteral("missing-legacy"));

  ProfileRepository repository;
  const ProfileSnapshot snapshot = snapshotFrom(repository.load(instance));

  QCOMPARE(snapshot.directory.hostPath, QDir(current).filePath(QStringLiteral("remmina")));
  QCOMPARE(snapshot.directoryFingerprint.symlinkParentPaths, QStringList({lexicalParent, nestedParent}));
  QVERIFY(profile_repository_detail::directoryMatches(snapshot.directory.hostPath, snapshot.directoryFingerprint));
}
...
void ProfileRepositoryTest::enforcesSymlinkTraversalLimit() {
  ...
  QTemporaryDir temporary;
  QVERIFY(temporary.isValid());
  const QString terminal = makeDirectory(QDir(temporary.path()).filePath(QStringLiteral("terminal")));
  const QString profilePath = makeDirectory(QDir(terminal).filePath(QStringLiteral("remmina")));
  ...
  for (int index = linkCount - 1; index >= 0; --index) {
    const QString parent = makeDirectory(QDir(temporary.path()).filePath(QStringLiteral("link-%1").arg(index)));
...
void ProfileRepositoryTest::parserMutationCannotPoisonCacheOrOpaqueIdentity() {
  ...
  ProfileRepository repository([&](const QString &source, const QString &launch, QString opaqueId) {
    ++parseCount;
    if (parseCount == 1) {
  ...
  QCOMPARE(firstLoad.profiles.constFirst().opaqueId,
           profile_repository_detail::opaqueProfileId(QStringView(instance.id), QStringView(secondIdentity)));
...
void ProfileRepositoryTest::distinguishesMissingFromUnreadableDirectory() {
  ...
  QCOMPARE(std::get<ProfileRepositoryError>(missing), ProfileRepositoryError::NoProfileDirectory);
  ...
  QCOMPARE(std::get<ProfileRepositoryError>(unreadable), ProfileRepositoryError::UnreadableDirectory);
  ...
  QCOMPARE(std::get<ProfileRepositoryError>(inaccessibleParent), ProfileRepositoryError::UnreadableDirectory);
  ...
  QCOMPARE(std::get<ProfileRepositoryError>(listOnly), ProfileRepositoryError::UnreadableDirectory);

  const QString protectedParent = makeDirectory(temporary.path() + QStringLiteral("/protected-target"));
  const QString protectedTarget = writeProfile(protectedParent, u"target.remmina");
  QVERIFY(QFile::link(protectedTarget, directory + QStringLiteral("/protected-link.remmina")));
...
void ProfileRepositoryTest::keepsOpaqueIdsStablePrivateAndUnambiguouslyFramed() {
  const QString first = profile_repository_detail::opaqueProfileId(QStringView(u"ab"), QStringView(u"c"));
  const QString repeated = profile_repository_detail::opaqueProfileId(QStringView(u"ab"), QStringView(u"c"));
  const QString framedDifferently = profile_repository_detail::opaqueProfileId(QStringView(u"a"), QStringView(u"bc"));
  const QString otherPath = profile_repository_detail::opaqueProfileId(QStringView(u"ab"), QStringView(u"d"));
  ...
  const QString privateHash = profile_repository_detail::opaqueProfileId(
      QStringView(u"synthetic-private-instance"), QStringView(u"/synthetic/private/profile-path.remmina"));
  ...
  QCOMPARE(QFileInfo(aliased.profiles.constFirst().sourcePath).fileName(), QStringLiteral("a.remmina"));

  instance.id = QStringLiteral("instance-two");
  const QString otherInstanceId = snapshotFrom(repository.load(instance)).profiles.constFirst().opaqueId;
...
void ProfileRepositoryTest::sizeAndModificationTimeChangesReparseOnlyChangedCandidate() {
  ...
  ProfileRepository repository([&counts](const QString &source, const QString &launch, QString opaqueId) {
...
void ProfileRepositoryTest::scopeChangesEvictPriorCache() {
  QTemporaryDir temporary;
  QVERIFY(temporary.isValid());
  RemminaInstance first = nativeInstance(temporary.path() + QStringLiteral("/first"), QStringLiteral("shared-id"));
  ...
  RemminaInstance otherDirectory =
      nativeInstance(temporary.path() + QStringLiteral("/second"), QStringLiteral("shared-id"));
