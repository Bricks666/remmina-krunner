...
ProfileRecord record(QString id = QStringLiteral("opaque-profile-a"), QString name = QStringLiteral("Office desktop"),
                     QString server = QStringLiteral("rdp.example.test"),
                     QString labelsDisplay = QStringLiteral("East, Admin"), QString protocol = QStringLiteral("RDP")) {
  return {
...
public:
  ...
  std::optional<ProfileRecord> resolve(QStringView expectedInstanceId, QStringView opaqueId) const override {
    ++resolveCalls;
...
struct Harness {
  explicit Harness(std::chrono::milliseconds timeout = 30s) : service(registry, catalog, launcher, timeout) {}
...
QDBusMessage await(QDBusPendingCall call) {
  ...
  if (!watcher.isFinished()) {
    return QDBusMessage::createError(QDBusError::Timeout, QStringLiteral("private match timed out"));
  }
...
void appendStrings(const QVariant &value, QStringList &strings) {
  if (value.metaType() == QMetaType::fromType<QString>()) {
    ...
    const RemminaKRunner::RemoteMatches matches = qdbus_cast<RemminaKRunner::RemoteMatches>(argument);
    for (const RemminaKRunner::RemoteMatch &match : matches) {
      appendStrings(QVariantList{match.id, match.text, match.iconName, match.categoryRelevance, match.relevance,
                                 match.properties},
                    strings);
    }
  }
...
void verifySafeProperties(const RemminaKRunner::RemoteMatch &match, const QString &subtext) {
  QCOMPARE(match.properties.keys(),
           QStringList({QStringLiteral("actions"), QStringLiteral("category"), QStringLiteral("subtext")}));
  QCOMPARE(match.properties.value(QStringLiteral("subtext")).metaType(), QMetaType::fromType<QString>());
  QCOMPARE(match.properties.value(QStringLiteral("subtext")).toString(), subtext);
  QCOMPARE(match.properties.value(QStringLiteral("category")).metaType(), QMetaType::fromType<QString>());
  QCOMPARE(match.properties.value(QStringLiteral("category")).toString(), QStringLiteral("Remmina"));
  QCOMPARE(match.properties.value(QStringLiteral("actions")).metaType(), QMetaType::fromType<QStringList>());
...
void verifyGenericError(const RemminaKRunner::RemoteMatches &matches, const QString &id) {
  ...
  verifySafeProperties(match, match.properties.value(QStringLiteral("subtext")).toString());
}
...
void RunnerServiceTest::ignoredAndCreationQueriesTouchNoSources() {
  Harness harness;
  for (const QString &query : {QString{}, QStringLiteral("   "), QStringLiteral("rem"), QStringLiteral("REM   "),
                               QStringLiteral("remote host"), QStringLiteral("rem:host")}) {
  ...
  const RemminaKRunner::RemoteMatches matches = harness.service.Match(QStringLiteral("ReM NeW"));
  QCOMPARE(matches.size(), 1);
  const RemminaKRunner::RemoteMatch &match = matches.constFirst();
...
void RunnerServiceTest::invalidSelectionsReturnGenericErrorWithoutCatalog() {
  const QList<RegistrySnapshot> snapshots{
      ...
      {{instance(QStringLiteral("duplicate")), instance(QStringLiteral("duplicate"))}, QStringLiteral("duplicate"), {}},
  };
  for (const RegistrySnapshot &snapshot : snapshots) {
    Harness harness;
    harness.registry.value = snapshot;
    verifyGenericError(harness.service.Match(QStringLiteral("rem office")), QStringLiteral("error:no-instance"));
...
void RunnerServiceTest::profileMatchesExposeOnlyVisibleMetadataAndStableOrdering() {
  ...
  const QString directTokenSentinel = QStringLiteral("direct-activation-token-privacy-sentinel");
  harness.service.SetActivationToken(directTokenSentinel);
  const RemminaKRunner::RemoteMatches matches = harness.service.Match(QStringLiteral("rem office east"));
  QCOMPARE(harness.registry.snapshotCalls, 1);
  ...
  for (const RemminaKRunner::RemoteMatch &match : matches) {
    variants.append(
        QVariantList{match.id, match.text, match.iconName, match.categoryRelevance, match.relevance, match.properties});
  }
  QDBusConnection bus = QDBusConnection::sessionBus();
  QVERIFY(bus.isConnected());
  const QString serviceName =
      QStringLiteral("org.example.RemminaKRunner.Privacy.p%1").arg(QCoreApplication::applicationPid());
  QVERIFY(bus.registerService(serviceName));
  QVERIFY(bus.registerObject(QStringLiteral("/runner"), &harness.service, QDBusConnection::ExportAllSlots));
  QDBusInterface interface(serviceName, QStringLiteral("/runner"), QStringLiteral("org.kde.krunner1"), bus);
  QVERIFY(interface.isValid());
  const QString liveTokenSentinel = QStringLiteral("live-activation-token-privacy-sentinel");
  harness.service.SetActivationToken(liveTokenSentinel);
  const QDBusMessage reply = await(interface.asyncCall(QStringLiteral("Match"), QStringLiteral("rem office east")));
  QCOMPARE(reply.type(), QDBusMessage::ReplyMessage);
  QCOMPARE(reply.arguments().size(), 1);
  const QDBusArgument encoded = qvariant_cast<QDBusArgument>(reply.arguments().constFirst());
  ...
  for (const QString &secret :
       {QStringLiteral("password=source-secret"), QStringLiteral("username-gateway-note-unknown-secret"),
        QStringLiteral("instance-password-secret"), QStringLiteral("/secret/gateway/remmina"),
        QStringLiteral("unknown-launcher-secret"), directTokenSentinel, liveTokenSentinel}) {
...
void RunnerServiceTest::subtitleOmitsBlankComponents() {
  ...
  const QList<Case> cases{
      ...
      {QStringLiteral("VNC"), QStringLiteral("  "), QStringLiteral(" lab "), QStringLiteral("VNC · lab")},
      {{}, {}, {}, {}},
  };
  for (const Case &item : cases) {
    Harness harness;
    harness.catalog.result = QList<ProfileRecord>{
        record(QStringLiteral("opaque"), QStringLiteral("Office"), item.server, item.labels, item.protocol)};
    const RemminaKRunner::RemoteMatches matches = harness.service.Match(QStringLiteral("rem office"));
    QCOMPARE(matches.size(), 1);
...
void RunnerServiceTest::successfulLookupsReuseSessionAndRestartIdleTimer() {
  ...
  QCOMPARE(harness.catalog.recordInstanceIds,
           QStringList({QStringLiteral("native:/usr/bin/remmina"), QStringLiteral("native:/usr/bin/remmina")}));
  QCOMPARE(harness.catalog.endCalls, 0);
...
void RunnerServiceTest::opaqueProfileIdsRemainActionableAcrossSessionCleanup() {
...

  {
    ...
    QCOMPARE(harness.launcher.ids, QStringList{validProfileA});
    harness.service.Run(QStringLiteral("action:new"), QString{});
    QCOMPARE(harness.launcher.tokens, QStringList({QStringLiteral("discard-replaced"), QString{}}));
  }
  ...
  Harness malformed;
  for (const QString &id : {QString(63, QLatin1Char('a')), QString(65, QLatin1Char('a')), QString(64, QLatin1Char('A')),
...
void RunnerServiceTest::matchExpiresAbandonedActivationToken() {
  ...
  for (const Case &item : cases) {
    ...
    for (const RemminaKRunner::RemoteMatch &match : matches) {
      serialized.append(QVariantList{match.id, match.text, match.iconName, match.categoryRelevance, match.relevance,
                                     match.properties});
    }
    QStringList strings;
...
void RunnerServiceTest::idleTimerIsAServiceChild() {
  Harness harness;
  const QList<QTimer *> timers = harness.service.findChildren<QTimer *>(QString{}, Qt::FindDirectChildrenOnly);
...
void RunnerServiceTest::selectionChangeBetweenMatchAndRunIsRejectedByLauncher() {
  ...
  RemminaLauncher launcher(registry, catalog, process, notifier, [] { return QProcessEnvironment{}; });
...
void RunnerServiceTest::configHasExactTriggerContractAndResetOrdering() {
  ...
  QCOMPARE(events, QStringList({QStringLiteral("end"), QStringLiteral("rescan"), QStringLiteral("reset")}));
  ...
  QCOMPARE(config.value(QStringLiteral("MatchRegex")).metaType(), QMetaType::fromType<QString>());

  const QRegularExpression regex(config.value(QStringLiteral("MatchRegex")).toString());
  QVERIFY(regex.isValid());
  for (const QString &accepted : {QStringLiteral("rem"), QStringLiteral("REM"), QStringLiteral("rem host"),
                                  QStringLiteral("rem new"), QStringLiteral("rem\thost")}) {
    QVERIFY2(regex.match(accepted).hasMatch(), qPrintable(accepted));
  }
  for (const QString &rejected : {QString{}, QStringLiteral("remote"), QStringLiteral("rem:host"),
                                  QStringLiteral("x rem host"), QStringLiteral(" rem host")}) {
  ...
  QCOMPARE(throwingEvents, QStringList({QStringLiteral("rescan"), QStringLiteral("reset")}));
}
...
void RunnerServiceTest::everyPublicSlotContainsDependencyExceptions() {
  ...
  verifyGenericError(snapshotFailure.service.Match(QStringLiteral("rem office")), QStringLiteral("error:internal"));
  ...
  verifyGenericError(recordFailure.service.Match(QStringLiteral("rem office")), QStringLiteral("error:internal"));
