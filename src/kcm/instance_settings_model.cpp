...
QString flatpakIdentity(const RemminaInstance &instance)
{
    ...
    return i18nc("@item:inlistbox Flatpak installation identity", "%1 · %2",
                 installation, ref);
}
...
QString displayText(const RemminaInstance &instance)
{
    ...
    case InstanceKind::Native:
        return i18nc("@item:inlistbox Remmina packaging and executable",
                     "Native — %1", oneLine(instance.executable));
    case InstanceKind::Flatpak:
        return i18nc("@item:inlistbox Remmina packaging and Flatpak identity",
                     "Flatpak — %1", flatpakIdentity(instance));
    case InstanceKind::Snap:
        return i18nc("@item:inlistbox Remmina packaging and executable",
                     "Snap — %1", oneLine(instance.executable));
    }
    return {};
}
...
InstanceSettingsModel::InstanceSettingsModel(InstanceRegistryControlSource &registry)
    : registry_(registry)
{
}
...
QList<SettingsInstanceItem> InstanceSettingsModel::items() const
{
    return items_;
}

QString InstanceSettingsModel::selectedId() const
{
    return selectedId_;
}

QString InstanceSettingsModel::persistedId() const
{
    return persistedId_;
}

QStringList InstanceSettingsModel::failedBackends() const
{
    return failedBackends_;
}

bool InstanceSettingsModel::hasInstances() const
{
    return !instances_.isEmpty();
}

bool InstanceSettingsModel::isDirty() const
{
    return selectedId_ != persistedId_;
}
