...
ScanEnvironment currentEnvironment()
{
    ...
    environment.pathEntries = qEnvironmentVariable("PATH").split(QDir::listSeparator(),
                                                                  Qt::SkipEmptyParts);
    environment.flatpakExecutable =
        QStandardPaths::findExecutable(QStringLiteral("flatpak"), environment.pathEntries);
...
struct RemminaRunnerConfig::ProductionDependencies {
    ProductionDependencies()
        : scanner(probe, currentEnvironment())
        , registry(scanner, store)
        , model(registry)
    {
    }
...
RemminaRunnerConfig::RemminaRunnerConfig(InstanceSettingsModel &model,
                                         QWidget *parent,
...
RemminaRunnerConfig::RemminaRunnerConfig(
    std::unique_ptr<ProductionDependencies> dependencies,
    QWidget *parent,
    const KPluginMetaData &data)
    : KCModule(parent, data)
    , dependencies_(std::move(dependencies))
    , model_(&dependencies_->model)
{
    initializeUi();
}
...
void RemminaRunnerConfig::save()
{
    if (!model_->save()) {
        ui_->statusLabel->setText(
            i18nc("@info", "Could not save the Remmina installation selection."));
        setDirtyState();
        return;
    }
...
void RemminaRunnerConfig::initializeUi()
{
    ...
    connect(ui_->instanceCombo, &QComboBox::currentIndexChanged,
            this, &RemminaRunnerConfig::instanceChanged);
}
...
void RemminaRunnerConfig::updateStatus()
{
    if (!lastScanSucceeded_) {
        ui_->statusLabel->setText(
            i18nc("@info", "Could not scan for Remmina installations."));
        return;
    }
    if (!model_->hasInstances()) {
        ui_->statusLabel->setText(
            i18nc("@info", "No Remmina installations found."));
        return;
    }
    ...
    if (!failedNames.isEmpty()) {
        ui_->statusLabel->setText(
            i18nc("@info", "Some installation types could not be checked: %1.",
