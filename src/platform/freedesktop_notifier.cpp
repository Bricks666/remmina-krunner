...
QDBusMessage freedesktop_notifier_detail::launchFailureMessage() {
  ...
  message << QString{launchFailureTitle} << uint{0} << QString{} << QString{launchFailureTitle}
          << QString{launchFailureBody} << QStringList{} << QVariantMap{} << -1;
  return message;
}
...
void FreedesktopNotifier::showLaunchFailure() noexcept {
  try {
    ...
    static_cast<void>(QDBusConnection::sessionBus().asyncCall(freedesktop_notifier_detail::launchFailureMessage(),
