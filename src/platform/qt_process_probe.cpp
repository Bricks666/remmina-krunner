...
void deferReaping(std::unique_ptr<QProcess> process) {
  ...
  QObject::connect(pendingProcess, &QProcess::finished, pendingProcess, &QObject::deleteLater);
}
...
void killAndReapOrDefer(std::unique_ptr<QProcess> &process) {
  ...
  process->kill();
  if (!process->waitForFinished(reapingGraceMilliseconds) && process->state() != QProcess::NotRunning) {
...
ProbeResult QtProcessProbe::run(const QString &executable, const QStringList &arguments) {
  QCoreApplication *application = QCoreApplication::instance();
  if (application == nullptr || QCoreApplication::closingDown() || QThread::currentThread() != application->thread()) {
  ...
  if (startWait <= 0 || !process->waitForStarted(startWait)) {
    const ProbeResult::Status status =
        deadline.hasExpired() ? ProbeResult::Status::TimedOut : ProbeResult::Status::Failed;
  ...
  while (process->state() != QProcess::NotRunning) {
    ...
    if (!process->waitForReadyRead(remainingTime) && process->state() != QProcess::NotRunning) {
