...
RemoteMatch errorMatch(QString id, QString subtext) {
  return {
      std::move(id), QStringLiteral("Remmina unavailable"), QString{iconName}, informationalMatch,
      0.0,           visibleProperties(std::move(subtext)),
  };
}
...
RemoteMatches noInstanceError() {
  return {errorMatch(QStringLiteral("error:no-instance"), QStringLiteral("No Remmina installation is available"))};
}

RemoteMatches noProfilesError() {
  return {errorMatch(QStringLiteral("error:no-profiles"), QStringLiteral("No Remmina profiles are available"))};
}

RemoteMatches unreadableError() {
  return {errorMatch(QStringLiteral("error:unreadable"), QStringLiteral("Remmina profiles cannot be read"))};
}

RemoteMatches internalError() {
  return {errorMatch(QStringLiteral("error:internal"), QStringLiteral("Unable to search Remmina profiles"))};
}
...
std::optional<RemminaInstance> selectedInstance(const RegistrySnapshot &snapshot) {
  ...
  return selected == nullptr ? std::nullopt : std::optional<RemminaInstance>{*selected};
}
...
RunnerService::RunnerService(InstanceRegistryControlSource &registry, ProfileCatalogAccess &catalog,
                             RemminaLaunchSource &launcher, QObject *parent)
    : RunnerService(registry, catalog, launcher, std::chrono::seconds(30), parent) {}

RunnerService::RunnerService(InstanceRegistryControlSource &registry, ProfileCatalogAccess &catalog,
                             RemminaLaunchSource &launcher, std::chrono::milliseconds catalogIdleTimeout,
                             QObject *parent)
    : QObject(parent), registry_(registry), catalog_(catalog), launcher_(launcher), catalogIdleTimer_(new QTimer(this)),
      creationResult_(creationResult()), noInstanceError_(noInstanceError()), noProfilesError_(noProfilesError()),
      unreadableError_(unreadableError()), internalError_(internalError()), config_(runnerConfig()) {
  ...
  connect(catalogIdleTimer_, &QTimer::timeout, this, [this] { endActiveSession(true); });
}

RunnerService::~RunnerService() noexcept { endActiveSession(true); }
...
RemoteMatches RunnerService::Match(const QString &query) noexcept {
  ...
  try {
    ...
    if (const auto *error = std::get_if<ProfileCatalogError>(&result)) {
      endActiveSession(false);
      return *error == ProfileCatalogError::NoProfileDirectory ? noProfilesError_ : unreadableError_;
    }

    const QList<SearchMatch> matches = matchProfiles(std::get<QList<ProfileRecord>>(result), parsed.tokens);
...
RunnerActions RunnerService::Actions() noexcept { return {}; }
...
void RunnerService::Run(const QString &matchId, const QString &actionId) noexcept {
  ...
  try {
    if (actionId.isEmpty() && !matchId.isEmpty() && !matchId.startsWith(errorPrefix)) {
      if (matchId == newActionId) {
        launcher_.create(QStringView{activationToken});
      } else if (!matchId.startsWith(actionPrefix) && isOpaqueProfileId(QStringView{matchId})) {
...
void RunnerService::Teardown() noexcept { teardownEveryCall(); }
