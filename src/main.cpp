...
ScanEnvironment scanEnvironment()
{
    ...
    return {
        ...
        .flatpakExecutable = QStandardPaths::findExecutable(
            QStringLiteral("flatpak"), pathEntries),
...
public:
    ProductionBackend()
        : scanner_(probe_, scanEnvironment())
        , registry_(scanner_, selectionStore_)
    {
    }

    RegistrySnapshot rescanAndRepair() override
    {
        return registry_.rescanAndRepair();
    }
    ...
    int startService(QCoreApplication &application) override
    {
        ...
        RemminaLauncher launcher(
            registry_, catalog, processLauncher, notifier);
        RemminaKRunner::RunnerService runner(
            registry_, catalog, launcher, &application);

        QDBusConnection bus = QDBusConnection::sessionBus();
        if (!bus.isConnected()
            || !bus.registerObject(QStringLiteral("/runner"),
                                   &runner,
...
int main(int argc, char *argv[])
{
    ...
    return RemminaKRunner::runApplication(application,
