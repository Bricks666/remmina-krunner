...
std::optional<InstanceKind> selectedKind(const RegistrySnapshot &snapshot) {
  ...
  return selected == nullptr ? std::nullopt : std::optional<InstanceKind>{selected->kind};
}
...
int runRescan(ApplicationBackend &backend, QTextStream &output) noexcept {
  try {
    ...
    const QString status = !snapshot.failedBackends.isEmpty() ? QStringLiteral("partial")
    ...
    output << QStringLiteral("scan_status=") << status << QLatin1Char('\n') << QStringLiteral("instances=")
           << snapshot.instances.size() << QLatin1Char('\n') << QStringLiteral("native=") << nativeCount
           << QLatin1Char('\n') << QStringLiteral("flatpak=") << flatpakCount << QLatin1Char('\n')
           << QStringLiteral("snap=") << snapCount << QLatin1Char('\n') << QStringLiteral("selected_type=")
           << (selection.has_value() ? kindName(*selection) : QStringLiteral("none")) << QLatin1Char('\n');
    output.flush();
...
int runApplication(QCoreApplication &application, const QStringList &arguments, ApplicationBackend &backend,
                   QTextStream &standardOutput, QTextStream &standardError) noexcept {
  const bool normalMode = arguments.size() == 1;
  const bool rescanMode = arguments.size() == 2 && arguments.at(1) == QStringLiteral("--rescan");
