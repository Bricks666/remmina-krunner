...
bool containsNul(QStringView value) { return value.contains(QChar::Null); }

bool validProgram(QStringView program) {
  return !program.isEmpty() && QDir::isAbsolutePath(program.toString()) && !containsNul(program);
}
...
RemminaLauncher::RemminaLauncher(InstanceRegistrySource &registry, ProfileCatalogReadSource &catalog,
                                 ProcessLauncher &processLauncher, Notifier &notifier)
    : RemminaLauncher(registry, catalog, processLauncher, notifier,
                      [] { return QProcessEnvironment::systemEnvironment(); }) {}

RemminaLauncher::RemminaLauncher(InstanceRegistrySource &registry, ProfileCatalogReadSource &catalog,
                                 ProcessLauncher &processLauncher, Notifier &notifier,
                                 EnvironmentProvider environmentProvider)
    : registry_(registry), catalog_(catalog), processLauncher_(processLauncher), notifier_(notifier),
...
RemminaLaunchResult RemminaLauncher::connect(QStringView opaqueId, QStringView activationToken) noexcept {
  try {
    RemminaInstance instance;
    if (!selectedInstance(instance)) {
      return failure(RemminaLaunchResult::NoInstance);
    ...
    const std::optional<ProfileRecord> record = catalog_.resolve(QStringView(instance.id), opaqueId);
    if (!record.has_value()) {
      return failure(RemminaLaunchResult::MissingProfile);
    }
    const QString launchPath = record->launchPath;
    if (launchPath.isEmpty() || !QDir::isAbsolutePath(launchPath) || containsNul(launchPath)) {
      return failure(RemminaLaunchResult::StartFailed);
    }
    QStringList arguments = instance.launcherPrefix;
...
RemminaLaunchResult RemminaLauncher::launch(const RemminaInstance &instance, QStringList arguments,
                                            QStringView activationToken) {
  if (!validProgram(instance.executable) || !validArguments(arguments) || containsNul(activationToken)) {
    return failure(RemminaLaunchResult::StartFailed);
  }
