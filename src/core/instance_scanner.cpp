...
QString childPath(const QString &parent, const QString &child) { return cleanPath(QDir(parent).filePath(child)); }
...
QString canonicalOrAbsolutePath(const QString &path) {
  ...
  return canonicalPath.isEmpty() ? cleanPath(fileInfo.absoluteFilePath()) : cleanPath(canonicalPath);
}
...
bool isUnderSnapRoot(const QString &canonicalPath, const QString &snapMountRoot) {
  ...
  return canonicalPath == snapMountRoot || canonicalPath.startsWith(snapMountRoot + QLatin1Char('/'));
}
...
ProfileEnvironment nativeProfiles(const QString &userHome) {
  const QString dataHome = cleanPath(QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation));
  ...
  return {
      .configHome = cleanPath(QStandardPaths::writableLocation(QStandardPaths::ConfigLocation)),
      .dataHome = dataHome,
...
bool isRemminaApplicationRef(const QString &ref) {
  ...
  const bool allComponentsPresent = std::all_of(components.cbegin(), components.cend(),
                                                [](const QString &component) { return !component.isEmpty(); });
...
std::optional<QString> decodeFlatpakField(const QByteArray &encodedField) {
  ...
  const bool containsControl = std::any_of(
      field.cbegin(), field.cend(), [](const QChar character) { return character.category() == QChar::Other_Control; });
  if (containsControl) {
    return std::nullopt;
  }
  field = field.trimmed();
...
QList<FlatpakInstallation> parseFlatpakInstallations(const QByteArray &output) {
  ...
  for (const QByteArray &line : output.split('\n')) {
    ...
    if (!application || !ref || !installation || *application != QLatin1StringView(remminaApplicationId) ||
  ...
  std::sort(installations.begin(), installations.end(),
            [](const FlatpakInstallation &left, const FlatpakInstallation &right) {
              ...
              const int installationOrder = QString::compare(left.installation, right.installation, Qt::CaseSensitive);
...
InstanceScanner::InstanceScanner(ProcessProbe &probe, ScanEnvironment environment)
    : probe_(probe), environment_(std::move(environment)) {}
...
InstanceScanResult InstanceScanner::scan() const {
  ...
  const QString snapLauncherPath =
      environment_.snapLauncher.isEmpty() ? QString{} : cleanPath(snapLauncherInfo.absoluteFilePath());
  ...
  for (const QString &pathEntry : environment_.pathEntries) {
    ...
    if ((!snapLauncherPath.isEmpty() && absolutePath == snapLauncherPath) ||
        ...
        isUnderSnapRoot(canonicalPath, snapMountRoot) || nativeCanonicalPaths.contains(canonicalPath)) {
      continue;
    }
  ...
  if (isExecutableFile(flatpakInfo)) {
    ...
    if (!flatpakCanonical.isEmpty()) {
      const ProbeResult probeResult =
          probe_.run(flatpakCanonical, {QStringLiteral("list"), QStringLiteral("--app"),
      ...
      if (probeResult.status != ProbeResult::Status::Success) {
        ...
        for (const FlatpakInstallation &installation : parseFlatpakInstallations(probeResult.standardOutput)) {
          result.instances.append({
              .id = QStringLiteral("flatpak:") + installation.installation + QLatin1Char(':') + installation.ref,
              .kind = InstanceKind::Flatpak,
              .displayName = QStringLiteral("Remmina (Flatpak: %1)").arg(installation.installation),
