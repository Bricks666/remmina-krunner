...
qint64 modifiedMilliseconds(const struct stat &metadata) {
  ...
  return static_cast<qint64>(metadata.st_mtimespec.tv_sec) * 1000 + metadata.st_mtimespec.tv_nsec / 1000000;
#else
  return static_cast<qint64>(metadata.st_mtim.tv_sec) * 1000 + metadata.st_mtim.tv_nsec / 1000000;
#endif
}
...
QString cacheKey(const RemminaInstance &instance, const LocatedProfileDirectory &directory,
...
QString canonicalIdentityForDescriptor(int descriptor) {
  const QByteArray descriptorPath = QByteArray("/proc/self/fd/") + QByteArray::number(descriptor);
  ...
  if (identity.endsWith(QStringLiteral(" (deleted)"), Qt::CaseSensitive) || !QDir::isAbsolutePath(identity)) {
    return {};
  }
...
std::optional<QStringList> symlinkParentPaths(const QString &lexicalPath) {
  ...
  while (!pending.isEmpty()) {
    ...
    do {
      targetLength = ::readlink(encodedPath.constData(), encodedTarget.data(), encodedTarget.size());
    ...
    QStringList targetComponents = target.split(QLatin1Char('/'), Qt::SkipEmptyParts);
...
std::optional<DirectoryFingerprint> fingerprintForDescriptor(int descriptor, const QString &lexicalPath) {
  struct stat metadata{};
  int result = -1;
  do {
...
CandidateInspection inspectCandidateAt(int directoryDescriptor, const LocatedProfileDirectory &directory,
                                       const QByteArray &encodedName, const QString &entryName) {
  struct stat metadata{};
  int statResult = -1;
  do {
    statResult = ::fstatat(directoryDescriptor, encodedName.constData(), &metadata, 0);
  } while (statResult < 0 && errno == EINTR);
  const int statError = statResult < 0 ? errno : 0;
  if (statResult < 0) {
    return {
        .candidate = std::nullopt,
        .directoryUnreadable = (statError == EACCES || statError == EPERM) && !directoryAccessible(directoryDescriptor),
    };
  }
  ...
  do {
    fileDescriptor = ::openat(directoryDescriptor, encodedName.constData(), O_RDONLY | O_NONBLOCK | O_CLOEXEC);
  ...
  } else if ((openError == EACCES || openError == EPERM) && !directoryAccessible(directoryDescriptor)) {
...
CandidateInspection inspectCandidate(const LocatedProfileDirectory &directory, const QString &entryName) {
  QByteArray encodedDirectory = QFile::encodeName(directory.hostPath);
  int descriptor = -1;
  do {
    descriptor = ::open(encodedDirectory.constData(), O_RDONLY | O_NONBLOCK | O_CLOEXEC | O_DIRECTORY);
  ...
  CandidateInspection inspection = inspectCandidateAt(descriptor, directory, encodedName, entryName);
  ::close(descriptor);
  return inspection;
}
...
EnumerationResult enumerateCandidates(const LocatedProfileDirectory &directory) {
  ...
  while (true) {
    ...
    CandidateInspection inspection = inspectCandidateAt(::dirfd(stream), directory, encodedName, entryName);
  ...
  std::sort(discovered.begin(), discovered.end(), [](const Candidate &left, const Candidate &right) {
    const int nameOrder = QString::compare(left.entryName, right.entryName, Qt::CaseSensitive);
    if (nameOrder != 0) {
      return nameOrder < 0;
    }
...
void normalizeRecord(ProfileParseResult &result, const Candidate &candidate, const QString &opaqueId) {
...
QString profile_repository_detail::opaqueProfileId(QStringView instanceId, QStringView canonicalIdentity) {
  return hashTuple({
...
std::optional<DirectoryFingerprint> profile_repository_detail::inspectDirectory(QStringView lexicalPath) {
  ...
  std::optional<DirectoryFingerprint> fingerprint = fingerprintForDescriptor(descriptor, cleaned);
  ::close(descriptor);
  return fingerprint;
}

bool profile_repository_detail::directoryMatches(QStringView lexicalPath, const DirectoryFingerprint &fingerprint) {
...
ProfileRepository::ProfileRepository(ProfileParserFunction parser) : parser_(std::move(parser)) {
  if (!parser_) {
...
RepositoryLoadResult ProfileRepository::load(const RemminaInstance &instance) {
  ProfileLocationResult locationResult = profile_locator_detail::locateProfileDirectoryDetailed(instance);
  if (const auto *error = std::get_if<ProfileLocationError>(&locationResult)) {
    ...
    return *error == ProfileLocationError::Unreadable ? ProfileRepositoryError::UnreadableDirectory
                                                      : ProfileRepositoryError::NoProfileDirectory;
  }
  const LocatedProfileDirectory directory = std::get<LocatedProfileDirectory>(std::move(locationResult));

  if (!hasActiveScope_ || activeInstanceId_ != instance.id || activeDirectory_ != directory.hostPath) {
    cache_.clear();
  ...
  for (const Candidate &enumeratedCandidate : candidates) {
    ...
    const bool cacheHit =
        cached != cache_.end() && cached->instanceId == instance.id && cached->directory == directory.hostPath &&
        cached->canonicalIdentity == candidate.canonicalIdentity && cached->device == candidate.signature.device &&
        cached->inode == candidate.signature.inode && cached->size == candidate.signature.size &&
    ...
    if (cacheHit) {
      ...
      const QString opaqueId = profile_repository_detail::opaqueProfileId(QStringView(instance.id),
                                                                          QStringView(candidate.canonicalIdentity));
    ...
    for (int attempt = 0; attempt < 2; ++attempt) {
      const QString opaqueId = profile_repository_detail::opaqueProfileId(QStringView(instance.id),
                                                                          QStringView(candidate.canonicalIdentity));
      ProfileParseResult parsed = parser_(candidate.sourcePath, candidate.launchPath, opaqueId);
      normalizeRecord(parsed, candidate, opaqueId);
    ...
    if (!stableResult.has_value()) {
      if (candidateStillEligible && !processedIdentities.contains(candidate.canonicalIdentity)) {
        processedIdentities.insert(candidate.canonicalIdentity);
        snapshot.fingerprint.append(candidate.fingerprint);
      }
      continue;
