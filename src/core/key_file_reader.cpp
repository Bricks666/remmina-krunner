...
bool isContinuationByte(unsigned char byte) { return byte >= 0x80 && byte <= 0xBF; }
...
bool isSyntacticWhitespace(char character) { return character == ' ' || character == '\t' || character == '\f'; }
...
bool isValidKeyName(QStringView key) {
  ...
  if (openingBracket == 0 || closingBracket != key.size() - 1 || key.indexOf(u'[', openingBracket + 1) >= 0) {
    return false;
  }

  const QStringView locale = key.sliced(openingBracket + 1, closingBracket - openingBracket - 1);
  if (locale.isEmpty()) {
    return false;
  }
...
ReadResult readAllowedKeyFileValuesWithError(const QString &path, QStringView section,
  ...
  while (!file.atEnd()) {
    ...
    if (lineCount > maximumLineCount || line.size() > maximumLineBytes ||
        totalBytes > maximumTotalBytes - line.size() || (!line.endsWith('\n') && !file.atEnd())) {
