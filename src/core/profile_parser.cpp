...
std::variant<ProfileRecord, ProfileParseError> parseRemminaProfile(const QString &sourcePath, const QString &launchPath,
  ...
  key_file_reader_detail::ReadResult readResult =
      key_file_reader_detail::readAllowedKeyFileValuesWithError(sourcePath, QStringView(u"remmina"), allowedKeys);
  if (const auto *error = std::get_if<key_file_reader_detail::ReadError>(&readResult)) {
    return *error == key_file_reader_detail::ReadError::Unreadable ? ProfileParseError::Unreadable
