...
ParsedQuery ignoredQuery() { return {QueryKind::Ignore, {}}; }
...
ParsedQuery parseRunnerQuery(QStringView query) {
  ...
  if (query.size() < triggerLength || query.first(triggerLength).toString().toCaseFolded() != QStringLiteral("rem")) {
    return ignoredQuery();
  }
...
QList<SearchMatch> matchProfiles(const QList<ProfileRecord> &profiles, const QStringList &tokens) {
  ...
  std::stable_sort(matches.begin(), matches.end(), [](const SearchMatch &left, const SearchMatch &right) {
    ...
    const int nameOrder = left.record.name.toCaseFolded().compare(right.record.name.toCaseFolded());
    if (nameOrder != 0) {
      return nameOrder < 0;
    }

    const int serverOrder = left.record.server.toCaseFolded().compare(right.record.server.toCaseFolded());
    if (serverOrder != 0) {
