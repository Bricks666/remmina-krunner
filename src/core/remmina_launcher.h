...
public:
  ...
  virtual RemminaLaunchResult connect(QStringView opaqueId, QStringView activationToken = {}) = 0;
};
...
public:
  ...
  RemminaLauncher(InstanceRegistrySource &registry, ProfileCatalogReadSource &catalog, ProcessLauncher &processLauncher,
                  Notifier &notifier);
  RemminaLauncher(InstanceRegistrySource &registry, ProfileCatalogReadSource &catalog, ProcessLauncher &processLauncher,
                  Notifier &notifier, EnvironmentProvider environmentProvider);

  RemminaLaunchResult create(QStringView activationToken = {}) noexcept override;
  RemminaLaunchResult connect(QStringView opaqueId, QStringView activationToken = {}) noexcept override;

private:
  [[nodiscard]] bool selectedInstance(RemminaInstance &instance) const;
  [[nodiscard]] RemminaLaunchResult launch(const RemminaInstance &instance, QStringList arguments,
